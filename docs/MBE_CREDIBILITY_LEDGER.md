...
| Oracle feasibility frontier | Completed known-truth diagnostic | 126,000/126,000 rows and 336 cells passed integrity gates; the observable oracle failed useful-power requirements at 24 configurations but passed calibration and power at 48, while learned degree-2 rules remained anti-conservative | develop a nuisance estimator that closes the 48-configuration oracle gap and confirm it on fresh known-truth data; this result does not authorize protected opening |
| Multi-target image transport atlas | Artifact complete; protected analysis locked | 360/360 valid CIFAR-10, CIFAR-100, and SVHN rows; 72 dataset-specific configuration blocks, five repeated seeds per block, three architectures per dataset, zero errors or duplicates, and matching structural/hash gates | this supports collection feasibility and supplies a prospective transport benchmark; it does not support survivor, washout, target-specific reliability, or SVHN transport claims until a rule passes the relevant 24-configuration known-truth gate |
