...
OUTPUT = Path("mbe3_corrected_image_factorial.csv")
MANIFEST = Path("mbe3_corrected_image_factorial_manifest.json")
INTEGRITY = Path("mbe3_corrected_image_factorial_integrity.json")
SPLIT_SEED = 20260811
...
def load_cifar(
    ...
    train_idx, val_idx, test_idx = _split_indices(n_train, n_val, n_test)
    try:
        ...
        val_subset = TensorDataset(torch.from_numpy(train_x[val_idx]), torch.from_numpy(train_y[val_idx]))
...
def frozen_grid(smoke: bool) -> list[RunConfig]:
    ...
    seeds = (8111,) if smoke else (8111, 8112)
    settings = [
        (optimizer, lr_level, learning_rates[optimizer][lr_level], wd, dropout)
    ...
    return [
        RunConfig(arch, optimizer, lr_level, lr, wd, dropout, seed)
...
def train_run(
    ...
    set_seed(cfg.seed)
    train_loader, val_loader, test_loader, metric_loaders = load_cifar(
        batch_size, n_train, n_val, n_test, cfg.seed
    )
    ...
    row: dict[str, float | int | str] = {
        ...
        "environment_id": "cifar10-fixed-splits-v1",
        "split_id": "cifar10-train-validation-test-20260811",
        "task": "cifar10",
        "arch": cfg.arch,
        "seed_id": cfg.seed,
    ...
    test_ev = evaluate(model, test_loader, device)
    ...
    row["test_loss"] = test_ev["loss"]
    row["test_acc"] = test_ev["acc"]
    row["final_acc"] = test_ev["acc"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Corrected CIFAR-10 MBE factorial")
    parser.add_argument("--smoke", action="store_true")
    parser.add_argument("--max-hours", type=float, default=10.5)
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--n-train", type=int, default=12_000)
    ...
    if args.n_train + args.n_val > 50_000 or args.n_test > 10_000:
        raise ValueError("requested split sizes exceed CIFAR-10")
    started = time.time()
    ...
    train_idx, val_idx, test_idx = _split_indices(args.n_train, args.n_val, args.n_test)
    manifest = {
        "schema_version": 1,
        "status": "preregistered_corrected_image_factorial",
        "experiment": "mbe3_corrected_image_factorial_v1",
        "environment_id": "cifar10-fixed-splits-v1",
        "split_id": "cifar10-train-validation-test-20260811",
        "split_hashes": {
        ...
        "primary_target": "test_loss",
        "secondary_target": "test_acc",
        "frozen_controls": [
            "arch",
            "optimizer",
            "lr_level",
        ...
        "primary_gate": "At least 90% of planned runs, all 48 configurations, both seeds per included configuration, and all three architectures. Otherwise descriptive only.",
    ...
    integrity = {
        ...
        "configurations_with_two_seeds": sum(count >= 2 for count in config_counts.values()),
        "architectures": sorted({row.get("arch") for row in valid}),
        "primary_gate_pass": (
            len(valid) >= math.ceil(0.9 * len(grid))
            and len(config_counts) == 48
            and all(count >= 2 for count in config_counts.values())
