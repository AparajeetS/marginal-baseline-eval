...
OUTPUT = Path("mbe2_corrected_causal_text_factorial.csv")
MANIFEST = Path("mbe2_corrected_causal_text_factorial_manifest.json")
...
def frozen_grid(smoke: bool) -> list[RunConfig]:
    ...
    # Ten preregistered optimizer/regularization interventions, shared by both
    # sizes. This varies each control without treating seed as a covariate.
    settings = [
        (1e-4, 0.0, 0.0),
        (1e-4, 1e-3, 0.1),
        (3e-4, 0.0, 0.0),
    ...
    seeds = [1701] if smoke else [1701, 1702, 1703, 1704, 1705]
    return [
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Frozen MBE 2.0 causal-language factorial")
    ...
    manifest = {
        "schema_version": 1,
        "status": "preregistered_primary_factorial",
        "experiment": "mbe2_corrected_causal_text_factorial_v1",
        "environment_id": "wikitext2-causal-lm-fixed-splits-v1-factorial",
        ...
        "primary_claim_scope": "Within WikiText-2, estimate metric reliability profiles for token NLL beyond frozen design and training-state baselines. Cross-environment or universal claims are out of scope.",
    ...
    for index, config in enumerate(grid, start=1):
        ...
        try:
            ...
            diagnostic_replicates = [
                diagnostic_metrics(
                    model, initial, splits["train"], args.sequence_length, device, config.seed + offset
                )
