...
Version 1 ended before corpus download or training because the assigned P100
was incompatible with the default PyTorch wheel. Version 2 changed only the
runtime compatibility bootstrap and reconciled the platform-normalized slug;
the grid, seeds, outcomes, controls, and preregistration were unchanged.
`FAILED_V1_SHA256SUMS` preserves the version-1 source identity.
